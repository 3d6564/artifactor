...
class SSHClient:
    ...
    def detect_os(self, host, jumpbox, jumpbox_username, target_username, jumpbox_password=None, jumpbox_key_path=None, target_password=None, target_key_path=None):
        os_command = 'uname -s'
        host, output = self.run_command_on_host(os_command, 
                                                host, 
                                                jumpbox, 
        ...
        if 'Linux' in output:
            ...
            host, output = self.run_command_on_host(os_command,
                                                    ...
                                                    target_key_path=target_key_path)
            if 'ID=' in output:
                id_line = next(line for line in output.splitlines() if line.startswith('ID='))
