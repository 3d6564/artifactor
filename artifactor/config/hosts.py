...
class HostManager:
    def __init__(self, hosts_file='host_file'):
        ...
        self.hosts = self.load_hosts(self.hosts_file)
    ...
    def load_hosts(self, hosts_file):
        try:
            ...
            print(f"\033[1;33mHosts file {hosts_file} does not exist... cloning template\033[0m")
            try:
                shutil.copyfile(self.host_template, hosts_file)
                with open(hosts_file, 'r') as f:
                    return [line.strip() for line in f.readlines() if line.strip()]
            except:
